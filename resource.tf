...
resource "aws_alb_target_group" "bastion_tg" {
  name        = "bastion-tg"
  port        = 22
  protocol    = "TCP"
  vpc_id      = aws_vpc.vpc.id
...
resource "aws_alb_target_group_attachment" "bastion_tg_attachment1" {
  target_group_arn = aws_alb_target_group.bastion_tg.arn
  target_id        = aws_instance.ec2_instance1.id
  port             = 22
}

resource "aws_alb_target_group_attachment" "bastion_tg_attachment2" {
  target_group_arn = aws_alb_target_group.bastion_tg.arn
  target_id        = aws_instance.ec2_instance2.id
  port             = 22
}

resource "aws_alb_listener" "bastion_alb_listener" {
  load_balancer_arn = aws_alb.bastion_alb.arn
  port              = "22"
  protocol          = "TCP"
